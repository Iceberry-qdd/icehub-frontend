...
import { callWebAuthn, getCurUserInfo, login, loginWithWebauthn } from '@/authApp/js/api.js'
...
async function autoComplete(abortController) {
    try {
...
async function tryGetCurUserInfo(){
    try{
        const response = await getCurUserInfo()
        if(response.ok) throw new Error('您已经登录过！')
    } catch(e){
        location = '/index.html'
    }
}

watch(() => state.turnstile.token, (newVal, _) => {
