<template>
    <div class="flex flex-col gap-y-4 p-4">
        ...
        <div v-if="showUnImpl" class="bg-blue-100 dark:bg-[#1e1e1e] p-4 rounded-[8px]">
            <!-- TODO implement it. -->
            <div class="flex flex-row items-center justify-between mb-2">
                <span class="font-bold text-[12pt]">猜你喜欢</span>
                <span class="cursor-pointer dark:text-blue-300 hover:underline hover:underline-offset-2 text-[10pt] text-blue-500">换一换</span>
            </div>
            <div
                v-for="user in state.users"
                :key="user.nickname"
                class="active:bg-blue-300/60 active:dark:bg-neutral-700 cursor-pointer flex flex-row gap-x-4 hover:bg-blue-200 hover:dark:bg-neutral-800 p-2 rounded-[8px]">
                <!-- eslint-disable-next-line vue/max-attributes-per-line -->
        ...
        <div v-if="state.threading.length > 0" class="bg-gray-50 dark:bg-[#1e1e1e] rounded-[8px]">
            ...
            <ol class="list-decimal list-inside">
                <li
                    ...
                    :key="value.rank"
                    class="active:bg-gray-200 active:dark:bg-neutral-700 hover:bg-gray-100 hover:dark:bg-neutral-800 hover:last:rounded-b-[8px] px-4 py-3 text-[11pt]"
                    @click="routeToSearch(value.key)">
                    <span>{{ value.key }}<span v-if="value.rank <= 3">🔥</span></span>
                    <!-- eslint-disable-next-line vue/singleline-html-element-content-newline -->
                    <div class="dark:text-neutral-500 pl-4 text-[10pt] text-gray-400">热度{{ humanizedNumber(value.score) }}</div>
                </li>
            </ol>
        </div>
    </div>
